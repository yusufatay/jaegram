#!/usr/bin/env python3
"""
Live test of Instagram bypass functionality.
...
async def test_instagram_service_bypass():
    ...
    # Test 1: Connection test with bypass
    print("\n🔗 Testing connection bypass...")
    try:
        result = await instagram_service.test_connection("testuser", "testpassword123")
        if result.get("success") and result.get("test_mode"):
            print("✅ Connection bypass: SUCCESS")
            print(f"   Message: {result.get('message')}")
            test_results.append(("Connection Bypass", True))
        else:
            print(f"❌ Connection bypass: FAILED - {result}")
            test_results.append(("Connection Bypass", False))
    except Exception as e:
        print(f"❌ Connection bypass: ERROR - {e}")
        test_results.append(("Connection Bypass", False))
    
    # Test 2: Profile data bypass
    print("\n👤 Testing profile data bypass...")
    try:
        result = await instagram_service.get_user_profile_data("testuser")
        if result.get("success") and result.get("test_mode"):
    ...
    # Test 3: Like action bypass
    print("\n❤️ Testing like action bypass...")
    try:
        result = await instagram_service.validate_like_action("testuser", "https://www.instagram.com/p/test123/")
        if result.get("success") and result.get("test_mode"):
    ...
    # Test 4: Follow action bypass
    print("\n👥 Testing follow action bypass...")
    try:
        result = await instagram_service.validate_follow_action("testuser", "target_user_123")
        if result.get("success") and result.get("test_mode"):
    ...
    # Test 5: Profile info bypass
    print("\n📊 Testing profile info bypass...")
    try:
        result = await instagram_service.get_profile_info("testuser")
        if result.get("success") and result.get("test_mode"):
            print("✅ Profile info bypass: SUCCESS")
            print(f"   Username: {result.get('profile_data', {}).get('username')}")
            print(f"   Media count: {result.get('profile_data', {}).get('media_count')}")
    ...
    # Test 6: Normal user validation (should fail)
    print("\n🚫 Testing normal user validation (should fail)...")
    try:
        result = await instagram_service.test_connection("normal_user", "normal_password")
        if not result.get("success") or not result.get("test_mode"):
            print("✅ Normal user validation: SUCCESS (correctly rejected)")
            print(f"   Error: {result.get('error', 'Connection failed as expected')}")
    ...
    # Print results
    print("\n" + "="*60)
    print("🎯 INSTAGRAM SERVICE BYPASS TEST RESULTS")
    print("="*60)
...
async def main():
    ...
    if service_success:
        print("✅ Instagram service bypass: WORKING")
    else:
        print("❌ Instagram service bypass: NOT WORKING")
